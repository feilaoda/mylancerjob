...
def parse_promotion_list(x, item):
    names = x.select('//div[@class="wrap_description"]/a/span[@style="display:inline-block"]/text()').extract()   
    if names:
        item['scrap_pn'] = names[0].strip()
...
def parse_product_list(x, item):
    names = x.select('//div[@class="itemText"]/div[@class="wrapper"]/a/span[@style="display:inline"]/text()').extract()   
    if names:
        item['scrap_pn'] = names[0].strip()
...
parsers = {
    ...
    'promotion_store_list': parse_promotion_list,
    'product_list': parse_product_list,
    'store_list': parse_store_list
}
